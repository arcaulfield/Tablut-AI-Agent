...
public class TablutBoardState extends BoardState {
    ...
    public List<Coord> getNeighbourPieces(Coord position, Piece colour)
    {
        ...
        if(position.y < BOARD_SIZE - 1 && getPieceAt(position.x, position.y + 1) == colour)
        {
            neighbours.add(Coordinates.get(position.x, position.y + 1));
        }
        else if(position.x < BOARD_SIZE - 1 && getPieceAt(position.x + 1, position.y) == colour)
        {
            neighbours.add(Coordinates.get(position.x + 1, position.y));
        }
        else if(position.x > 0 && getPieceAt(position.x - 1, position.y) == colour)
        {
            neighbours.add(Coordinates.get(position.x - 1, position.y));
        }
        else if(position.y > 0 && getPieceAt(position.x, position.y - 1) == colour)
        {
            neighbours.add(Coordinates.get(position.x, position.y - 1));
        }
        return neighbours;
